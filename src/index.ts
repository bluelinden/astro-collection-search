...
export interface SearchIndexOptions {
	...
	/**
	 * defaults to /\.mdx{0,1}$/i. filters the content in the content folders by file extension or other pattern.
	 */
	customRegex?: RegExp;
}
...
const ReadCollection = async (
	...
	try {
		for await (const entry of ListFiles(dir)) {
			if ((customRegex ?? /\.mdx{0,1}$/i).test(entry)) {
...
const RebuildSearchIndex = async (
	...
	if (docs.length) {
		...
		const artifacts_directory = path.join(
			options.outputDirectory ?? "public",
		...
		await fs.copyFile(
			path.join(import.meta.dirname, "..", "dist", worker),
			path.join(
				options.outputDirectory ?? "public",
				constants.artifacts_directory,
